...
const Profile = () => {
    ...
    const [profileUser, setProfileUser] = useState<z.infer<typeof UserSchema> | null>(null);
    const [profileUserFav, setProfileUserFav] = useState<any[]>([]);
    ...
    const fetchProfileUserFav = useCallback(async (userId: string) => {
        try {
            const { data } = await getFavProducts(userId);
            setProfileUserFav(data);
        } catch (error) {
    ...
    useEffect(() => {
        if (profileUser?.id) {
            fetchProfileUserFav(profileUser.id);
        }
    }, [profileUser?.id, fetchProfileUserFav]);

    const favoriteItems = useMemo(() => (
        profileUserFav?.length > 0
            ? profileUserFav.slice(0, 5).map(fav => (
