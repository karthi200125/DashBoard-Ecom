                ...
                <div className='max-w-full md:w-[1000px] p-5'>
                    ...
                    <div className='flex flex-col md:flex-row gap-5 justify-between'>
                        ...
                        <div className='w-full md:w-[650px] flex flex-wrap gap-5 justify-between overflow-y-auto py-3'>
                            <CustomInput name='name' inputCls='flex-1' label='User Name' type='text' value={user?.name} />
                            <CustomInput name='email' inputCls='flex-1' label='Email' type='email' value={user?.email} />
                            <CustomInput name='phoneNo' inputCls='flex-1' label='Mobile Number' type='number' value={user?.phoneNo} />
                            <CustomInput name='address' inputCls='flex-1' label='Permanent Address' type='text' value={user?.address} />
                            <CustomInput name='postalCode' inputCls='flex-1' label='Postal Code' type='number' value={user?.postalCode} />
                            <div className='flex flex-col md:flex-row items-center gap-5 w-full'>
                                <CustomSelect
                                    name="city"
                                    control={methods.control}
                                    defaultValue={user?.city || "Select City Name"}
                                    options={cities}
                                    label="Select City"
                                    selectCls="w-full"
                                ...
                                <CustomSelect
                                    name="state"
                                    control={methods.control}
                                    defaultValue={user?.state || "Select State Name"}
                                    options={states}
                                    label="Select State"
                                    selectCls="w-full"
                                    errors={methods.formState.errors}
